...
이어서 보여주는 사이트입니다. 첫 화면(랜딩 페이지)에서 "학생 시작하기"를 누르면
아이디+이름을 입력해 입장하고, **과목(문법 영상 / 독해 지문 영상) → 시리즈(교재) →
챕터 → 강의** 순서로 찾아보며 학습합니다. 이미 본 영상은 완료 표시가 되고, 과목별로
"이어보기"가 자동 추천됩니다.

## 주요 기능

- **랜딩 페이지**: 첫 화면에 "학생 시작하기" 버튼과 선생님용 관리 페이지 링크 표시
- **로그인**: 학생이 아이디 + 이름을 직접 입력해 입장 (아이디·이름이 모두 일치해야 로그인됨)
...
- **관리자 페이지** (`/admin`, 비밀번호 보호): 시리즈/챕터/영상 추가·수정·삭제, 학생(아이디+이름) 관리, 영상 링크 등록
...
### 시드에 들어있는 학생 아이디 (예시 — 꼭 바꿔서 쓰세요)

`lib/seedData.ts` 상단 `STUDENTS` 배열에 학생별 로그인 아이디가 들어있습니다.
실제 운영 전에 `npm run db:seed` 전에 원하는 아이디로 수정하거나, `/admin`에서
학생을 지우고 새로 등록해서 원하는 아이디로 바꿀 수 있습니다.
...
- 인증: 쿠키 기반 세션 (학생: 아이디 + 이름, 관리자: 비밀번호)
...
Student(학생: 아이디 + 이름) ─ Progress(시청 여부, 복습 횟수) ─ Video
```
...
app/page.tsx                 랜딩 페이지 (학생 시작하기 / 관리자 링크)
app/login/                   학생 로그인 화면 (아이디 + 이름)
